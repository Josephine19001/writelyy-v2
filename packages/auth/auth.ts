...
import { updateSeatsInWorkspaceSubscription } from "./lib/workspace";
	...
	plugins: [
		...
		organization({
			sendInvitationEmail: async (
				...
				await sendEmail({
					to: email,
					templateId: "workspaceInvitation",
					locale,
					context: {
...
export type Workspace = typeof auth.$Infer.Organization;

export type WorkspaceMemberRole = ActiveWorkspace["members"][number]["role"];

export type WorkspaceInvitationStatus = typeof auth.$Infer.Invitation.status;

export type WorkspaceMetadata = Record<string, unknown> | undefined;
