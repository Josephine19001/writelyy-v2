			...
			<div className="flex flex-col items-center justify-center h-full p-8 text-center">
				<div className="max-w-md">
					...
					<h2 className="text-2xl font-semibold mb-2">
						{t("workspaces.dashboard.noWorkspaces", { default: "No workspaces yet" })}
					</h2>
					<p className="text-muted-foreground mb-6">
						{t("workspaces.dashboard.createFirstWorkspace", { 
							default: "Create your first workspace to get started with collaborative writing." 
						})}
					</p>
					{config.workspaces.enableUsersToCreateWorkspaces && (
						<Link href="/new-workspace">
							<Button variant="primary" size="lg">
								...
								{t("workspaces.dashboard.createWorkspace", { default: "Create Workspace" })}
							</Button>
						</Link>
					)}
				</div>
		...
		<div className="p-6 space-y-6 h-full overflow-y-auto">
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					<h1 className="text-3xl font-bold tracking-tight">
						{t("workspaces.dashboard.title", { default: "Workspaces" })}
					</h1>
				...
				{config.workspaces.enableUsersToCreateWorkspaces && (
					<Link href="/new-workspace">
						<Button variant="primary">
							<Plus className="h-4 w-4 mr-2" />
							{t("workspaces.dashboard.createNew", { default: "New Workspace" })}
						</Button>
					</Link>
				)}
			</div>
			...
			<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div className="relative flex-1 max-w-2xl">
					...
					<Input
						placeholder={t("workspaces.dashboard.searchPlaceholder", { 
					...
					<div className="relative">
						...
						>
							<option value="updated">
								{t("workspaces.dashboard.sortByUpdated", { default: "Recently Updated" })}
							</option>
							<option value="created">
								{t("workspaces.dashboard.sortByCreated", { default: "Recently Created" })}
							</option>
							<option value="name">
								{t("workspaces.dashboard.sortByName", { default: "Name" })}
							</option>
						</select>
					</div>
				...
				<div className="@container">
					<div className="grid @2xl:grid-cols-3 @lg:grid-cols-2 grid-cols-1 gap-4">
						{filteredWorkspaces?.map((workspace) => (
							...
							>
								<Link href={`/app/${workspace.slug}`} className="flex items-center gap-4 w-full">
	...
	if (!workspaces?.length) {
		return (
			<Card className="p-8 text-center">
				<p className="text-muted-foreground">
					{t("workspaces.dashboard.noWorkspacesFound", { default: "No workspaces found" })}
				</p>
			</Card>
		);
	}
		...
		<Card>
			<div className="divide-y">
				{workspaces.map((workspace) => (
					...
					>
						<Link href={`/app/${workspace.slug}`} className="flex items-center gap-3 flex-1">
