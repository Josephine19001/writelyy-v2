...
export function NavBar() {
	...
	const basePath = activeWorkspace ? `/app/${activeWorkspace.slug}` : "/app";
	...
	const menuItems = [
		...
		},
		{
			...
			href: activeWorkspace
				? `/app/${activeWorkspace.slug}/chatbot`
				: "/app/chatbot",
			icon: BotMessageSquareIcon,
			isActive: pathname.includes("/chatbot"),
		},
		...(activeWorkspace && !config.workspaces.hideWorkspace
			? [
					{
						label: t("app.menu.workspaceSettings"),
				...
				<div className="flex flex-wrap items-center justify-between gap-4">
					...
					>
						...
						{config.workspaces.enable &&
							!config.workspaces.hideWorkspace && (
								<>
									<span
										className={cn(
