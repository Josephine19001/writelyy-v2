...
const WorkspaceCacheContext = createContext<WorkspaceCacheContextValue | null>(null);

export function useWorkspaceCacheContext() {
  const context = useContext(WorkspaceCacheContext);
  if (!context) {
    throw new Error("useWorkspaceCacheContext must be used within a WorkspaceCacheProvider");
  }
  return context;
}
...
export function WorkspaceCacheProvider({ children }: WorkspaceCacheProviderProps) {
  ...
  }), [cacheHook.documents.length, cacheHook.folders.length, cacheHook.sources.length]);

  // Aggressive prefetching strategy
  useEffect(() => {
    if (!activeWorkspace?.id || cacheHook.isLoading) return;
  ...
  useEffect(() => {
    if (!activeWorkspace?.id || !cacheHook.documents.length) return;
  ...
  const enhancedTrackDocumentAccess = React.useCallback((documentId: string) => {
    ...
    let accessData: Record<string, { count: number; lastAccess: number }> = {};
    if (existing) {
      try {
  ...
  const warmCache = React.useCallback(() => {
    ...
    if (accessData) {
      try {
        ...
        console.warn("Failed to warm cache from access patterns:", error);
      }
    }
