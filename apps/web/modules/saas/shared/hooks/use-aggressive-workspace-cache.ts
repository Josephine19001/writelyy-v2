...
export function useAggressiveWorkspaceCache(organizationId: string | null) {
  ...
  const workspaceDataQuery = useQuery({
    ...
    queryFn: async (): Promise<WorkspaceData> => {
      ...
      console.log("🚀 Loading ALL workspace data for:", organizationId);
      ...
      console.log(`✅ Workspace data loaded in ${(endTime - startTime).toFixed(2)}ms`);
  ...
  useEffect(() => {
    setCacheState({
      isInitializing: workspaceDataQuery.isLoading && !workspaceDataQuery.data,
      ...
      lastUpdated: workspaceDataQuery.dataUpdatedAt ? new Date(workspaceDataQuery.dataUpdatedAt) : null,
    });
  }, [workspaceDataQuery.data, workspaceDataQuery.isLoading, workspaceDataQuery.error, workspaceDataQuery.dataUpdatedAt]);
  ...
  useEffect(() => {
    ...
    console.log("📦 Populating individual query caches...");
    ...
    console.log(`✅ Cached ${documents.length} docs, ${folders.length} folders, ${sources.length} sources`);
  ...
  useEffect(() => {
    ...
    const prefetchTopDocuments = async () => {
      ...
      console.log(`🔥 Prefetching ${documentsToPreload.length} priority documents...`);
      ...
      console.log("✅ Priority documents prefetched");
    };
  ...
  const accessDocument = useCallback(async (documentId: string) => {
  ...
  const updateDocument = useCallback((documentId: string, updates: Partial<any>) => {
    if (!organizationId) return;

    // Update individual document cache
    queryClient.setQueryData(["document", documentId], (oldDoc: any) => ({
      ...oldDoc,
      ...updates,
    ...
    queryClient.setQueryData(["documents", organizationId], (oldData: any) => {
    ...
    queryClient.setQueryData(["workspaceData", organizationId], (oldData: WorkspaceData | undefined) => {
      if (!oldData) return oldData;
